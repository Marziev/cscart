...
class OrderService
{
    ...
    public function updateYandexOrderStatusByShipment($shipment_id)
    {
        ...
        if (empty($yandex_order_status['status'])) {
            return;
        }
    ...
    protected function updateYandexOrderStatus($id, $status_code)
    {
        ...
        if (empty($order_statuses[$status_code])) {
            return;
        }
    ...
    {
        $request = new OperationResult(false);
        ...
        if (
            !$delivery_options
            || !is_array($delivery_options)
            || isset($delivery_options['message'])
        ) {
            /** @psalm-suppress ArgumentTypeCoercion */
            $request = $this->client->processErrors($request, $delivery_options);
            return $request;
        }
        $delivery_options = reset($delivery_options);
        $request_data['externalId'] = $data['order_id'];
        if (!empty($data['comment'])) {
            $request_data['comment'] = $data['comment'];
        }
        $request_data['recipient'] = $this->request_builder->buildOrderRecipient($data, $order_info);
        $request_data['contacts'] = $this->request_builder->buildOrderContacts($data);
        $request_data['cost'] = $this->request_builder->buildOrderCost($data, $delivery_options, $order_info['status']);
        $request_data['places'] = $this->request_builder->buildOrderPlaces($order_info, $shipment);
        if (!$request_data['places']) {
            fn_set_notification(NotificationSeverity::ERROR, __('error'), __('yandex_delivery_v3.your_products_without_parameters'));
            return $request;
        }
        $request_data['deliveryOption'] = $this->request_builder->buildOrderDeliveryOption($order_info, $delivery_options);
        if (!$is_draft) {
            $request_data['shipment'] = $this->request_builder->buildOrderShipment($data, $order_info, $delivery_options);
        }
