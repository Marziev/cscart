...
class YandexDelivery implements IService, IPickupService
{
    ...
    /** @var int $courier_point_id Current courier point id.  */
    public $courier_point_id = 0;

    /** @var int $pickup_point_id Current pickup point id. */
    public $pickup_point_id = 0;
    ...
    public function prepareData($shipping_info)
    {
        ...
        } elseif (isset(Tygh::$app['session']['cart']['shipping'][$group_key]['point_id'])) {
            $this->courier_point_id = Tygh::$app['session']['cart']['shipping'][$group_key]['point_id'];
        }
        ...
        if (isset(Tygh::$app['session']['cart']['selected_yad_office'][$this->company_id][$shipping_id]['pickup_point_id'])) {
            ...
            $this->pickup_point_id = Tygh::$app['session']['cart']['shipping'][$group_key]['point_id'];
        }
    }
    ...
    /**
     * @param array<string, string>|string $response       Response information from Yandex.Delivery API.
     ...
     */
    protected function processPickupPoints($response, array $service_params)
    {
     ...
     * @throws \Exception DateTime related exception.
     */
    protected function processCourierPoints($response, array $service_params)
    {
        $result = [
            'cost'          => false,
            'error'         => false,
