...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update_offers') {
        ...
        && !empty($_REQUEST['price_ids'])
        && !empty($_REQUEST['status'])
    ) {
        $status_to = $_REQUEST['status'];

        foreach ((array) $_REQUEST['price_ids'] as $price_id) {
        ...
        if (defined('AJAX_REQUEST')) {
            $redirect_url = fn_url('yml.manage');
    ...
    return [CONTROLLER_STATUS_OK, 'yml' . $suffix];
}
...
if ($mode === 'offers_params') {
    ...
    if (!empty($_REQUEST['price_id'])) {
        ...
        if (Registry::get('runtime.company_id') === 0) {
            ...
            /** @var \Tygh\Storefront\Storefront $storefront */
            $storefront = $repository->findAvailableForCompanyId(Registry::get('runtime.company_id'), false);
            if (!empty($storefront) && is_array($storefront)) {
                $storefront = reset($storefront);
            }
        }

        if (isset($storefront->storefront_id)) {
            $price_list['param_data']['storefront_id'] = $storefront->storefront_id;
        }
