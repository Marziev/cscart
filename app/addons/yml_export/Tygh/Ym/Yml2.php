...
class Yml2 implements IYml2
{
    ...
    protected function generateOffers($file_body, $file_categories)
    {
        ...
        $storefront = $repository->findById((int) $this->options['storefront_id']);
    ...
    {
        $secure_storefront = Settings::instance()->getSettingDataByName(
            'secure_storefront',
            $this->company_id,
            CART_LANGUAGE,
            $this->options['storefront_id']
        );

        if (fn_allowed_for('MULTIVENDOR')) {
            if ((int) $this->company_id !== 0) {
                $url = fn_url(
                    "companies.view?company_id={$this->company_id}?storefront_id={$this->options['storefront_id']}",
                    SiteArea::STOREFRONT,
                    YesNo::toBool($secure_storefront['value'])
                        ? 'https'
                        : 'http'
                );
                $email = db_get_field('SELECT email FROM ?:companies WHERE company_id = ?i', $this->company_id);
                $company = fn_get_company_name($this->company_id);
            } else {
                $url = Registry::get('settings.Company.company_website');
                $email = Registry::get('settings.Company.company_orders_department');
                $company = Registry::get('settings.Company.company_name');
            }
        } else {
            $url = $secure_storefront['value'] === YesNo::YES
                ? Registry::get('config.https_location')
